...
const $fn = {
  color: {
    error: {
      throw: (colorValue: string) => { throw new Error(`${EAnkhColorParserError.InvalidFormat}: ${colorValue}`) },
    },
    getUnit: (colorValue: string) => {
      if (colorValue.startsWith("#")) return EAnkhColorUnit.Hex;
      if (colorValue.startsWith("rgb(") && colorValue.endsWith(")")) return EAnkhColorUnit.Rgb;
      if (colorValue.startsWith("lab(") && colorValue.endsWith(")")) return EAnkhColorUnit.Lab;
      if (colorValue.startsWith("hsl") && colorValue.endsWith(")")) return EAnkhColorUnit.Hsl;
      $fn.color.error.throw(colorValue);
    },
    getNumValues: (colorValue: string) => {
      try {
        /** @todo separate all color units to define better regex */
        const numArray = colorValue.match(/^[a-z]{3}\((-?[\d\.]+%?),? ?(-?[\d\.]+%?),? ?(-?[\d\.]+%?)\)$/)?.slice(1) || [];
        return numArray.map((num: string) => parseInt(num, 10));
      }
      catch (error: any) { $fn.color.error.throw(colorValue) }
    },
    isNumberArray: (maybeNumberArray: unknown[]) => {
      if (!maybeNumberArray.length) return false;
      try {
        return maybeNumberArray.every((maybeNumber) => !isNaN(maybeNumber as number) && !!(maybeNumber as number).toString().length);
    ...
    parse: {
      hex: (colorValue: string) => {
        if (!$fn.color.validate.hex(colorValue)) $fn.color.error.throw(colorValue);
        const colorVal = colorValue.toLowerCase();
        return colorVal.startsWith("#") ? colorVal.slice(1) : colorVal;
      },
      hsl: (colorValue: string) => {
        if (!$fn.color.validate.hsl(colorValue)) $fn.color.error.throw(colorValue);
        const numValues = $fn.color.getNumValues(colorValue);
        return numValues || [];
      },
      lab: (colorValue: string) => {
        if (!$fn.color.validate.lab(colorValue)) $fn.color.error.throw(colorValue);
        const numValues = $fn.color.getNumValues(colorValue);
        return numValues || [];
      },
      rgb: (colorValue: string) => {
        if (!$fn.color.validate.rgb(colorValue)) $fn.color.error.throw(colorValue);
        const numValues = $fn.color.getNumValues(colorValue);
        return numValues || [];
      },
      stringValue: (colorValue: string) => {
        ...
        switch (parsed.unit) {
          case EAnkhColorUnit.Hex:
            if (!$fn.color.validate.hex(colorValue)) $fn.color.error.throw(colorValue);
            return { ...parsed, parsedValue: $fn.color.parse.hex(colorValue), value: colorValue };
          case EAnkhColorUnit.Hsl:
            if (!$fn.color.validate.hsl(colorValue)) $fn.color.error.throw(colorValue);
            return { ...parsed, parsedValue: $fn.color.parse.hsl(colorValue), value: colorValue };
          case EAnkhColorUnit.Lab:
            if (!$fn.color.validate.lab(colorValue)) $fn.color.error.throw(colorValue);
            return { ...parsed, parsedValue: $fn.color.parse.lab(colorValue), value: colorValue };
          case EAnkhColorUnit.Rgb:
            if (!$fn.color.validate.rgb(colorValue)) $fn.color.error.throw(colorValue);
            return { ...parsed, parsedValue: $fn.color.parse.rgb(colorValue), value: colorValue };
          default: $fn.color.error.throw(colorValue);
        }
      },
...
export function ColorHue({ color: value }: IAnkhUiColorHue) {
  const [currentColor, setCurrentColor] = useState<IAnkhColor>($fn.color.parse.stringValue(value)!);
  ...
  return (
    <Auth.ReadRole >
      <div data-ui="color-hue" className="colorHue">
        {hueItems.map((hueItem, i) => <ColorHueItem key={`color-hue-item-${i}`} value={hueItem.value} />)}
      </div>
    </Auth.ReadRole >
  );
}
